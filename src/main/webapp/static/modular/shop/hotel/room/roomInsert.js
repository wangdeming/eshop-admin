...
function addSubmit() {
    ...
    $.ajaxSetup({crossDomain: true, xhrFields: {withCredentials: true}});
    $.ajax({
        url: Feng.ctxPath +"/shop/hotel/insert",
        type: 'POST',
        dataType: 'json',
        data: {name:name,breakfast:breakfast,broadband:broadband,window:window,area:area,bedWidth:bedWidth,price:price,person:person,canCancel:canCancel,imageListJsonString:imageListJsonString,content:content},
        async: false,
        success: function (data) {
            if(data.code != 200){
                Feng.error(data.message);
