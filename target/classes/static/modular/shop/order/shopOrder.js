...
$(function () {
    ...
    $(".mybt").click(function () {
        $("#createStartTime").val(getDate(-$(this).attr('datas')));
        $("#createEndTime").val(getDate(-$(this).attr('datae')));
...
function getOrderList() {
    ...
    $.ajax({
        url: Feng.ctxPath +"/shop/order/orderList",
        type: 'POST',
        dataType: 'json',
        data: orderData,
        async: false,
                ...
                for (var i=0;i<orderDate.length;i++){
                    ...
                    for(var j=0;j<orderDate[i].goods.length;j++){
                        ...
                        if (orderDate[i].status == 2){
                            sendGoodsHtml = '<span style="display: inline-block;width: 100%;line-height: 30px;">等待商家发货</span><a style="display: inline-block;width: 100%;line-height: 30px;" onclick="sendGoods('+ orderDate[i].id +')">发货</a>';
                        ...
                        if(j==0){
                            ...
                            orderTbodyHtml += '<tr style="height: 50px;line-height: 50px;"><td style="display: flex;padding: 10px;border-top: 1px solid #808080;border-right: none;border-bottom: none;border-left: none;" class="borderSty"><img style="width: 60px;height: 40px;margin-right: 5px;float: left;" src="'
