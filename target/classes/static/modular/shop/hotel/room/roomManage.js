...
function getRoomSettingList(dateStr) {
    ...
    var ajax = new $ax(Feng.ctxPath + "/shop/hotel/roomSettingList", function (data) {
        ...
        var theadHtml = '<th></th>';
        var tbodyHtml = '<tr style="text-align: center;"><td>'+ name +'</td>';
        for (var i=0;i<roomSettingList.length;i++){
            ...
            var status = '';
            if(roomSettingList[i].status != null){
                status = roomSettingList[i].status;
            }
            var statusNum = 0;
            if(status == '开'){
                statusNum =1;
            ...
            var price = '';
            if(roomSettingList[i].price != null){
                price = roomSettingList[i].price;
            }
            var number = '';
            if(roomSettingList[i].number != null){
                number = roomSettingList[i].number;
            }
            var numOrder = '';
            if(roomSettingList[i].numOrder != null){
                numOrder = roomSettingList[i].numOrder;
            }
            ...
            if(Date.parse(tempDate)<Date.parse(getDay(0)) || timeValue>90){
                ...
                tbodyHtml += '<td style="cursor: pointer;" onclick="goRoomManage('+ statusNum +','+ price +','+ atime +')">' +
                    '<div style="height: 30px;line-height: 30px;">'+ status +'</div>' +
                    '<div style="height: 30px;line-height: 30px;">￥'+ price +'</div>' +
                    '<div style="height: 30px;line-height: 30px;">'+ number +'</div>' +
                    '<div style="height: 30px;line-height: 30px;position: relative;" onmouseover="mouseOver(event,'+ numOrder +')" onmouseout="mouseOut(event)">' +
...
//房态设置
function goRoomManage(statusNum,price,atime) {
    ...
    $.sendConfirm({
        ...
        onBeforeConfirm: function() {
            ...
            if (!(/^\d+(?=\.{0,1}\d+$|$)/.test(roomPrice)) || roomPrice < 1 || roomPrice > 100000) {
